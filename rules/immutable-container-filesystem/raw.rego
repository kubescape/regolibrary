...
deny[msga] {
    ...
    result := is_mutable_filesystem(container, beginning_of_path, i)
...
is_mutable_filesystem(container, beginning_of_path, i) = [failed_path, fixPath]  {
	container.securityContext.readOnlyRootFilesystem == false
	failed_path = sprintf("%vcontainers[%v].securityContext.readOnlyRootFilesystem", [beginning_of_path, format_int(i, 10)])
	fixPath = ""
 }
 ...
 is_mutable_filesystem(container, beginning_of_path, i)  = [failed_path, fixPath] {
	...
	fixPath = {"path": sprintf("%vcontainers[%v].securityContext.readOnlyRootFilesystem", [beginning_of_path, format_int(i, 10)]), "value": "true"}
	failed_path = ""
 }
