...
deny[msga] {
    ...
    result := is_allow_privilege_escalation_container(container, i, beginning_of_path)
...
is_allow_privilege_escalation_container(container, i, beginning_of_path) = [failed_path, fixPath] {
    not container.securityContext.allowPrivilegeEscalation == false
	not container.securityContext.allowPrivilegeEscalation == true
	psps := [psp |  psp= input[_]; psp.kind == "PodSecurityPolicy"]
	count(psps) == 0
	failed_path = ""
	fixPath = {"path": sprintf("%vcontainers[%v].securityContext.allowPrivilegeEscalation", [beginning_of_path, format_int(i, 10)]), "value":"false"} 
}
...
is_allow_privilege_escalation_container(container, i, beginning_of_path) = [failed_path, fixPath]  {
	...
	count(psps) == 0
	fixPath = ""
	failed_path = sprintf("%vcontainers[%v].securityContext.allowPrivilegeEscalation", [beginning_of_path, format_int(i, 10)])
}
