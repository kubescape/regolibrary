...
deny[msga] {
    ...
    result := is_root_container(container, beginning_of_path, i)
	msga := {
...
deny[msga] {
	...
	container := pod.spec.containers[i]
	beginning_of_path := ""
    result := is_root_pod(pod, container, beginning_of_path)
	msga := {
		"alertMessage": sprintf("container: %v in pod: %v  runs with id less than 1000", [container.name, pod.metadata.name]),
		"packagename": "armo_builtins",
		"alertScore": 7,
...
deny[msga] {
	...
	spec_template_spec_patterns[wl.kind]
    container := wl.spec.template.spec.containers[i]
	beginning_of_path := "spec.template."
    result := is_root_pod(wl.spec.template, container, beginning_of_path)
    msga := {
		"alertMessage": sprintf("container :%v in %v: %v runs with id less than 1000", [container.name, wl.kind, wl.metadata.name]),
		"packagename": "armo_builtins",
		"alertScore": 7,
...
deny[msga] {
	...
	wl.kind == "CronJob"
	container = wl.spec.jobTemplate.spec.template.spec.containers[i]
	beginning_of_path := "spec.jobTemplate.spec.template."
    result := is_root_pod(wl.spec.jobTemplate.spec.template, container, beginning_of_path)
    msga := {
		"alertMessage": sprintf("container :%v in %v: %v runs with id less than 1000", [container.name, wl.kind, wl.metadata.name]),
		"packagename": "armo_builtins",
		"alertScore": 7,
...
is_root_pod(pod, container, beginning_of_path) = path {
	...
	path = sprintf("%vspec.securityContext.runAsUser", [beginning_of_path])
}
...
is_root_pod(pod, container, beginning_of_path) = path {
	...
	path = sprintf("%vspec.securityContext.runAsGroup", [beginning_of_path])
}
...
is_root_pod(pod, container, beginning_of_path) = path {
    pod.spec.securityContext.runAsGroup < 1000
	 pod.spec.securityContext.runAsUser < 1000
	path = sprintf("%vspec.securityContext", [beginning_of_path])
}


is_root_container(container, beginning_of_path, i) = path {
    container.securityContext.runAsUser < 1000
	not container.securityContext.runAsGroup
	path = sprintf("%vcontainers[%v].securityContext.runAsUser", [beginning_of_path, format_int(i, 10)])
}

is_root_container(container, beginning_of_path, i) = path {
    container.securityContext.runAsGroup < 1000
	not container.securityContext.runAsUser
	path = sprintf("%vcontainers[%v].securityContext.runAsGroup", [beginning_of_path, format_int(i, 10)])
}